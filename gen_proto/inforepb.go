...
const (
	...
	grpcxPackage   = protogen.GoImportPath("github.com/vison888/go-vkit/grpcx")
)
...
func generateFile(gen *protogen.Plugin, file *protogen.File) {
	filename := file.GeneratedFilenamePrefix + ".infore.pb.go"
	g := gen.NewGeneratedFile(filename, file.GoImportPath)
	g.P("// Code generated by protoc-gen-infore. DO NOT EDIT.")
	g.P("// versions:")
	g.P(fmt.Sprintf("// - protoc-gen-infore %s", release))
...
type %sClient struct {
	...
	cc grpcx.Client
		...
		for _, method := range service.Methods {
			...
			// logger.Infof("method.Desc.Name():%s", method.Desc.Name())
			methodDescPath := fmt.Sprintf("%s.%s", service.GoName, method.Desc.Name())
